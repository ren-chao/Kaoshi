layui.extend({
   /* dtree: '{/}js/lay-module/layui_ext/dtree/dtree'   */// {/}的意思即代表采用自有路径，即不跟随 base 路径
    ...
    var tableIns = table.render({
    	 elem: '#test',
         url:'/layui/l?action=seleRoleAll',
        toolbar: '#toolbarDemo',
        page : true,
        method:"post",
        height: 'full-145',
    		...
    		success:function(layero, index){
    			$.ajax({
    				...
    				success:function(data){
    					...
    					/*body.find("#uname2").val(info.username);*/
    					body.find("#uname").val(info.loginname);
    					...
    					/*------下拉框赋值--------*/
    					$.ajax({
    						  url:"/layui/qu?action=selectType",
    						  type:"post",
    						  async: false,
    						  success:function(data){
    							  var info = eval("("+data+")");
    							  ...
    							  $.ajax({
    								  url:"/layui/qu?action=seleOne",	//查询用户是否有角色有返回1,没有返回0
    								  data:{"userid":userid},
    								  type:"post",
    								  success:function(data){
    									  var i = eval("("+data+")");
    ...
    function deleteuser(userid){
    	$.ajax({
    		url:"/layui/qu?action=deleteuser",
    		data:{"userid":userid},
    		type:"post",
    		success:function(data){
    			if(data == 1){
