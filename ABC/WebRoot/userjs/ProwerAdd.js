layui.use(['form','layer','layedit','laydate','upload'],function(){
    ...
    form.on('submit(menuUser)',function(data){
		...
		$.post("http://localhost:8080/ABC/qu?action=insertquan&name="+mname+"&function="+mfunction+"&type="+type2+"&mbth1="+mbtn+"&fatherid="+father,function(data){
			var info = eval("("+data+")");
	 		if(info == 1){
	 			top.layer.msg("权限添加成功");
				parent.layer.closeAll("iframe");
				parent.layui.treeTable.reload('demoTreeTb');
	 		}else{
    ...
    form.on('select(type2)', function(data){
        ...
        if(type == 3){
        	...
        	$.get("http://localhost:8080/ABC/qu?action=sellebutton",function (res) {
        		var cs = eval('(' + res + ')');
        		var btntype = $('#btnType');
        		var html = '<span value="0"></span>';
                $.each(cs,function (index, item) {
                    html += '<span class="btnType4" onclick="tongyiType(this)">'+item.name+'</span> ';
                });
                btntype.html(html);
        	})
        }else{
        ...
        $.get("http://localhost:8080/ABC/qu?action=seletype&type="+(type-1),function (res) {
        	var cs = eval('(' + res + ')');
            var data = cs.data;
            var dom = $("#father");
            if(type == 1){
